...
+ Buckhorn Cholla: *Cylindropuntia anthrocarpa*
+ Silver Cholla: *Cylindropuntia enchinocarpa*
+ Beavertail: *Opuntia basilaris*

![Buckhorn Cholla, *Cylindropuntia anthrocarpa*](buckhorn.jpg)
...
#bring in data
cholla <- read_csv("~/Masters/Masters_cactus_Ch2/Baseline_Cactus_Size/baseline_cactus_size_cholla.csv")
opuntia <- read_csv("~/Masters/Masters_cactus_Ch2/Baseline_Cactus_Size/baseline_cactus_size_opuntia_basilaris.csv")
buckhorn <- read_csv("~/Masters/Masters_cactus_Ch2/Baseline_Cactus_Size/baseline_cactus_size_cylindropuntia_anthrocarpa.csv")
silver <- read_csv("~/Masters/Masters_cactus_Ch2/Baseline_Cactus_Size/baseline_cactus_size_cylindropuntia_echinocarpa.csv")
...
####We were able to sample 98 Silver Cholla (*C. enchinocharpa*) and 105 Buckhorn Cholla (*C. anthrocarpa*), but had to do nearly twice as many transects for the former (9 versus 5 transects). You can see the differences in densities for the two cholla species in the map above. However, we had to abandon the transect method and search haphazardly to find any Beavertail (*Opuntia basilaris*) at all. *C. anthrocarpa* is the densest cactus species at Sunset Cove, followed by *C. enchinocarpa* and then *O. basilaris*. Based on my proposed protocol, each study species needs 150 individuals to replicate each combination of variables 10 times. Ideally, the individuals manipulated between flowering and fruiting season will not be resampled in the the fruiting season, as our manipulation of the flowers in April may impact the number of fruit in August. This means that *C. anthrocarpa* is a solid study species option, *C. enchinocarpa* is certainly possible, but not as dominant as its cousin, and *O. basilaris* is out of the question. 
...
ggplot(buckhorn, aes(x = z)) +
  geom_density() + labs(title = "Size of Cacti", x = "Buckhorn", y = "Height in meters") +
  theme_minimal() #bimodal, ever so slightly
ggplot(silver, aes(x = z)) +
  geom_density() + labs(title = "Size of Cacti", x = "Silver", y = "Height in meters") +
  theme_minimal() #unimodal
ggplot(opuntia, aes(x = z)) +
  geom_density() + labs(title = "Size of Cacti", x = "Opuntia", y = "Height in meters") +
  theme_minimal()
...
#Make it pretty
#create vectors
species <- c("Cylindropuntia anthrocarpa", "Cylindropuntia enchinocarpa", "Opuntia basilaris")
...
ggplot(all, aes(x = health_index)) +
  geom_histogram() + labs(title = "Health of Cacti", x = "Number of individuals in each health class by species", y = "Health class") +
  facet_grid(~species) +
  theme_minimal()
